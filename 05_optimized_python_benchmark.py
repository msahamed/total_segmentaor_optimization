...
def postprocess(prediction, img_resampled, img_canonical, original_img):
    ...
    if orig_axcodes != canon_axcodes:
        ...
        seg_img_final = nib.Nifti1Image(seg_img_canonical.get_fdata().astype(
            np.uint8), original_img.affine, header=original_img.header)
    ...
    for i, (category, img_path) in enumerate(datasets):
        ...
        try:
            ...
            img_resampled = change_spacing(
                img_canonical, new_spacing=3.0, order=1)
            ...
            prediction = prediction[:, :, :original_shape_resampled[0],
                                    :original_shape_resampled[1], :original_shape_resampled[2]]
            ...
            print(
                f"  Latency: {total_time:.2f}s (Pre: {pre_time:.2f}s, Inf: {inf_time:.2f}s, Post: {post_time:.2f}s)")
